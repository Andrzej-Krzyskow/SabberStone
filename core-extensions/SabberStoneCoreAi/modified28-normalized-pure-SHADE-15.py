...
def shade_observer(generation, population, population_fitness, mean_f=0.5, mean_cr=0.5):
	...
	if generation == 0:
		...
		_log_files['stats'] = open('shade-pure-15-modified28-normalized-statistics-file-{}.csv'.format(ts), 'w')
		_log_files['indiv'] = open('shade-pure-15-modified28-normalized-individuals-file-{}.csv'.format(ts), 'w')
		_log_files['matrix'] = open('shade-pure-15-modified28-normalized-matrix-file-{}.csv'.format(ts), 'w')
		_log_files['coeff'] = open('shade-pure-15-modified28-normalized-coefficients-file-{}.csv'.format(ts), 'w')
