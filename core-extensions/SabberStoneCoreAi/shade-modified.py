...
def improve(fun, run_info, dimension, check_evals, name_output=None, replace=True, popsize=100, H=100,
	...
	observer   : optional callable(generation: int, population: np.ndarray,
								   population_fitness: np.ndarray, mean_f: float, mean_cr: float)
	...
	pmin = 2.0 / popsize
	generation = 0
	total_updates = 0  # <--- NEW: Track how many times the memory has actually been updated
	...
	while currentEval < maxEval:
		...
		if observer is not None:
			...
			if total_updates == 0:
				...
				# Table not full yet: only average the slots we've written to!
				current_mean_f = MemF[:total_updates].mean()
				current_mean_cr = MemCR[:total_updates].mean()
			else:
				# Table is full: average the whole thing
	...
	# Print the smart mean at the very end as well
