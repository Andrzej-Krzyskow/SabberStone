...
def generate_weights(random, args):
    ...
    offset1 = 0
    offset2 = NUM_WEIGHTS_PER_PHASE
    for i in range(NUM_WEIGHTS_PER_PHASE):
        shift = random.uniform(-PHASE2_SHIFT, PHASE2_SHIFT)
        weights[offset2 + i] = shift_weight(weights[offset1 + i] + shift)

    offset3 = 2 * NUM_WEIGHTS_PER_PHASE
    for i in range(NUM_WEIGHTS_PER_PHASE):
        shift = random.uniform(-PHASE3_SHIFT, PHASE3_SHIFT)
        weights[offset3 + i] = shift_weight(weights[offset2 + i] + shift)
...
def launch_simulator(f1, f2, d1, d2, thread_id):
	...
	if test:
		...
		command_line = r"dotnet run --project D:\Pics_Movies\vids\PWr\Sem10\magisterka\MODIFIED63S_PARALLEL_HS\SabberStone" + thread_id + r"\core-extensions\SabberStoneCoreAi\SabberStoneCoreAi.csproj"
