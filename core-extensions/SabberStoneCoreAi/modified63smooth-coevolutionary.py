...
def decode_weights(genes):
    decoded = [0.0] * NUM_WEIGHTS

    for i in range(NUM_WEIGHTS_PER_PHASE):
        p1 = clamp01(genes[i])

        d12 = (genes[NUM_WEIGHTS_PER_PHASE + i] * 2.0 - 1.0) * MAX_SHIFT_12
        p2 = clamp01(p1 + d12)

        d23 = (genes[2 * NUM_WEIGHTS_PER_PHASE + i] * 2.0 - 1.0) * MAX_SHIFT_23
        p3 = clamp01(p2 + d23)

        decoded[i] = p1
        decoded[NUM_WEIGHTS_PER_PHASE + i] = p2
        decoded[2 * NUM_WEIGHTS_PER_PHASE + i] = p3
...
def my_file_observer(population, num_generations, num_evaluations, args):
	...
	try:
		...
		# Replaced 'inspyred' with the ALG_NAME variable
		statistics_file = open(f'{ALG_NAME}-statistics-file-{time.strftime("%m%d%Y-%H%M%S")}.csv', 'w')
	...
	try:
		...
		# Replaced 'inspyred' with the ALG_NAME variable
		individuals_file = open(f'{ALG_NAME}-individuals-file-{time.strftime("%m%d%Y-%H%M%S")}.csv', 'w')
		args['individuals_file'] = individuals_file
	try:
		matrix_file = args['matrix_file']
	except KeyError:
		# Replaced 'inspyred' with the ALG_NAME variable
		matrix_file = open(f'{ALG_NAME}-matrix-file-{time.strftime("%m%d%Y-%H%M%S")}.csv', 'w')
...
def run_one(prng=None, display=False):
	...
	if display:
		...
		print('Best Solution (raw):\n{0}'.format(str(best)))

		decoded = decode_weights(best.candidate)
		print('Best Solution (decoded):\n{0}'.format(decoded))
