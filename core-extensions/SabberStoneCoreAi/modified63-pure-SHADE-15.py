...
OPPONENTS_CSV_ORIGINAL = "opponents-15-modified63.csv"
...
def shade_observer(generation, population, population_fitness, mean_f=0.5, mean_cr=0.5):
	...
	if generation == 0:
		...
		_log_files['stats'] = open('shade-pure-15-modified63-statistics-file-{}.csv'.format(ts), 'w')
		_log_files['indiv'] = open('shade-pure-15-modified63-individuals-file-{}.csv'.format(ts), 'w')
		_log_files['matrix'] = open('shade-pure-15-modified63-matrix-file-{}.csv'.format(ts), 'w')
		_log_files['coeff'] = open('shade-pure-15-modified63-coefficients-file-{}.csv'.format(ts), 'w')
...
def run_one():
    ...
    ts = time.strftime('%m%d%Y-%H%M%S')
    _run_opponents_csv = "opponents-15-modified63-{}.csv".format(ts)
