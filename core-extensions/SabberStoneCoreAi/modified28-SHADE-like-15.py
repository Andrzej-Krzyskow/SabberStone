...
def shade_observer(generation, population, population_fitness, mean_f=0.5, mean_cr=0.5):
    ...
    if generation == 0:
        ...
        _log_files['stats']  = open('shade-like-15-modified28-statistics-file-{}.csv'.format(ts),   'w')
        _log_files['indiv']  = open('shade-like-15-modified28-individuals-file-{}.csv'.format(ts),  'w')
        _log_files['matrix'] = open('shade-like-15-modified28-matrix-file-{}.csv'.format(ts),       'w')
        _log_files['coeff']  = open('shade-like-15-modified28-coefficients-file-{}.csv'.format(ts), 'w')
