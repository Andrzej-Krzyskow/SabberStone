...
NUM_GAMES = 20 #8
POP_SIZE = 15 #10
MAX_EVALUATIONS = 525 #1000
NUM_THREADS = 128  # 8
NUM_WEIGHTS = 63
TEMP_FILE_NAME = "results.tmp"
TEST_DUMMY = False

OPPONENTS_CSV_ORIGINAL = "opponents-15-modified63-smooth.csv"
...
def shade_observer(generation, population, population_fitness, mean_f=0.5, mean_cr=0.5):
	...
	if generation == 0:
		...
		_log_files['stats'] = open('shade-pure-15-modified63-smooth-statistics-file-{}.csv'.format(ts), 'w')
		_log_files['indiv'] = open('shade-pure-15-modified63-smooth-individuals-file-{}.csv'.format(ts), 'w')
		_log_files['matrix'] = open('shade-pure-15-modified63-smooth-matrix-file-{}.csv'.format(ts), 'w')
		_log_files['coeff'] = open('shade-pure-15-modified63-smooth-coefficients-file-{}.csv'.format(ts), 'w')
...
def run_one():
    ...
    ts = time.strftime('%m%d%Y-%H%M%S')
    _run_opponents_csv = "opponents-15-modified63-smooth-{}.csv".format(ts)
