...
Najważniejszy wynik jest jednoznaczny: wariant \textit{21 głębokościowy} uzyskał najwyższą skuteczność w fazie II i utrzymał przewagę nad agentem bazowym także w fazie III. Zaproponowane modyfikacje nie są więc tylko formalnym rozszerzeniem modelu --- realnie poprawiają decyzje turniejowe.

Uzyskane wyniki pokazują, że połączenie adaptacyjnej optymalizacji ewolucyjnej z komponentem planowania głębokościowego jest skutecznym kierunkiem rozwoju agentów dla gier karcianych o niepełnej informacji. Praca dostarcza praktyczne rozwiązanie i spójne podstawy do dalszych badań nad modelami hybrydowymi.
...
Odpowiedź jest warunkowo pozytywna. Sama rozbudowa przestrzeni poszukiwań do 63 parametrów nie gwarantuje poprawy --- wariant \textit{63 parametry} bez mechanizmu łagodzenia przejść był minimalne słabszy od agenta bazowego i wyraźnie słabszy dla pozostałych wariantów. Wariant \textit{63 płynny} pokazuje natomiast, że reprezentacja fazowa jest użyteczna, gdy łączy się ją ze stabilizacją przejść między etapami gry.
...
Odpowiedź jest pozytywna i jest to najsilniejszy efekt w całej pracy. Wariant \textit{21 głębokościowy} uzyskał najwyższy wynik turniejowy w fazie II, mimo że optymalizował ten sam zestaw 21 wag co agent bazowy. W fazie III osiągnął też wyraźnie lepszy rezultat przeciw agentowi Sebastiana Millera niż wariant bazowy. Największy zysk jakościowy wynika więc ze zmiany horyzontu decyzyjnego, a nie ze zwiększania liczby parametrów funkcji oceny.
