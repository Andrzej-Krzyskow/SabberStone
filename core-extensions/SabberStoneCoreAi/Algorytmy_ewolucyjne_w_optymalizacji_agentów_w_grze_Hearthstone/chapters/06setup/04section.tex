...
Rzetelne porównanie wariantów algorytmów wymaga przydzielenia każdemu z nich identycznego budżetu obliczeniowego. Wyrównanie wyłącznie według liczby wywołań funkcji przystosowania jest niewystarczające --- w schemacie koewolucyjnym jedno wywołanie ewaluatora ocenia pulę $2\mu$ osobników metodą każdy z każdym, generując $\binom{2\mu}{2} \cdot 9$ wywołań symulatora (9 kombinacji talii), natomiast w SHADE z Hall of Fame analogiczny koszt wynosi $N_{\text{opp}} \cdot 9$. Wyrównanie tylko po liczbie wywołań funkcji przystosowania prowadziłoby do przydzielenia bardzo różnej mocy obliczeniowej poszczególnym konfiguracjom.
...
\begin{equation}
    B_0 = \frac{\mu(\mu-1)}{2} \cdot 9, \qquad
    B_{\text{evo}} = \mu(2\mu - 1) \cdot 9
    \label{eq:bevo}
\end{equation}
Całkowita liczba wywołań symulatora:
\begin{equation}
    B_{\text{koew}} = B_0 + G_{\text{evo}} \cdot B_{\text{evo}},
...
Każdy osobnik oceniany jest względem $N_{\text{opp}}$ stałych przeciwników we wszystkich kombinacjach talii. Co $K_{\text{HOF}} = 5$ generacji mechanizm Hall of Fame ponownie ewaluuje $\mu$ rodziców, a koszt tej reewaluacji nie jest wliczany do licznika \texttt{check\_evals}. Łączna liczba wywołań funkcji przystosowania wynosi:
\begin{equation}
    F_{\text{total}} = \mu + \texttt{check\_evals}
    + \left\lfloor \frac{\texttt{check\_evals}}
      {I_{\text{HOF}} \cdot \mu} \right\rfloor \cdot \mu
    \label{eq:ftotal}
\end{equation}
Łączna liczba wywołań symulatora:
\begin{equation}
    B_{\text{HoF}} = F_{\text{total}} \cdot N_{\text{opp}} \cdot 9
...
\begin{equation}
    B^* = \frac{10 \cdot 9}{2} \cdot 9
        + 99 \cdot 10 \cdot 19 \cdot 9
    = 405 + 169 290
    = \mathbf{169 695 \text{ wywołań symulatora}}
    \label{eq:bstar}
\end{equation}
co odpowiada $3 393 900$ rozegranych partii Hearthstone ($B^* \cdot 20$).
...
\begin{equation}
    ...
    \label{eq:check_coev}
\end{equation}
Dla SHADE z Hall of Fame wartość wyznaczana jest iteracyjnie jako największa liczba całkowita spełniająca warunek $F_{\text{total}} \leq F^*_{\text{total}}$, gdzie $F^*_{\text{total}} = B^* / (9 \cdot N_{\text{opp}})$.
