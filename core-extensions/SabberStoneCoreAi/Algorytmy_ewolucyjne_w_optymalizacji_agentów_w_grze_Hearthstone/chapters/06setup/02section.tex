...
W sekcji przedstawiono pięć modyfikacji agenta bazowego, zaprojektowanych w celu poprawy jakości decyzji turniejowych. Zmiany obejmują zarówno rozszerzenie reprezentacji stanu, jak i modyfikacje strategii podejmowania decyzji. Każdy wariant opisano oddzielnie wraz z uzasadnieniem projektowym i formalizacją.

\subsection{Wariant 28 parametrów --- rozszerzony zestaw cech}
\label{sec:agent28}

Funkcja oceny agenta bazowego uwzględnia tylko cechy bezpośrednio związane ze stronnikami i statystykami bohatera, pomijając inne aspekty stanu gry. Pierwszy wariant rozszerza tę funkcję o siedem nowych cech kontekstowych --- ich wybór uzasadniono przy każdej definicji.
...
\begin{split}
    f_{28}(s, a, \mathbf{w}) =\;&
        f(s, a, \mathbf{w}) \\
        +\;& \Delta \text{Br}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Br}^{\text{wł}}(s,a,\mathbf{w})
        + \Delta \text{Ręka}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Ręka}^{\text{wł}}(s,a,\mathbf{w}) \\
        +\;& \Delta \text{Zak}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Zak}^{\text{wł}}(s,a,\mathbf{w})
        - \Delta \text{Prz}^{\text{wł}}(s,a,\mathbf{w}) \\
        +\;& \Delta \text{Smok}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Smok}^{\text{wł}}(s,a,\mathbf{w})
        + \Delta \text{Jade}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Jade}^{\text{wł}}(s,a,\mathbf{w}) \\
        +\;& \Delta \text{Pir}^{\text{prz}}(s,a,\mathbf{w}) - \Delta \text{Pir}^{\text{wł}}(s,a,\mathbf{w})
\end{split}
    \label{eq:score28}
\end{equation}
gdzie $f(s, a, \mathbf{w})$ jest funkcją bazową \eqref{eq:score}. Argumenty $(s,a,\mathbf{w})$ nowych składników wiążemy ze stanami graczy $p^{\text{wł}}$, $p^{\text{prz}}$ w $s$ oraz $p'^{\text{wł}}$, $p'^{\text{prz}}$ w $s'=T(s,a)$ tak jak w sekcji~\ref{sec:formalization} --- np.\ $\Delta\text{Br}^{\text{wł}}(s,a,\mathbf{w}) := \Delta\text{Br}(p^{\text{wł}}, p'^{\text{wł}}, \mathbf{w})$. To samo dotyczy pozostałych cech, z wyjątkiem przeciążenia many, które występuje wyłącznie po stronie własnego gracza. Szczegółowe wzory podano poniżej.

\paragraph{Trwałość broni $\Delta \text{Br}$.}

Broń bohatera jest stałym źródłem obrażeń przez wiele tur --- każde uderzenie zużywa jeden punkt trwałości. Utrata trwałości przez przeciwnika lub utrzymanie własnej broni ma bezpośredni wpływ na tempo (ang.~\textit{tempo}) gry, szczególnie w taliach agresywnych, takich jak \textit{AggroPirateWarrior} \cite{sanchez2020}. Cecha ta pozwala agentowi doceniać posiadanie broni i karać jej brak.
\begin{equation}
    \Delta \text{Br}(p, p', \mathbf{w}) =
        w_{\text{Br}} \cdot \bigl(\text{wtrz}(p) - \text{wtrz}(p')\bigr)
\end{equation}
gdzie $\text{wtrz}(p)$ to trwałość (ang.~\textit{durability}) broni bohatera gracza $p$ (0, jeśli brakuje broni).
...
Bonus do obrażeń zaklęć (ang.~\textit{spell damage}) wzmacnia wszystkie zaklęcia zadające obrażenia. Obecność stronników lub artefaktów przyznających ten bonus zwiększa potencjał ofensywny talii Maga i Szamana, umożliwiając np. jednorazowe usunięcie przeciwnych stronników. Śledzenie tej cechy pozwala agentowi premiować pozycje z aktywnym bonusem.
\begin{equation}
...
\end{equation}
gdzie $\text{smoki}(p)$ to liczba kart rasy smok w ręce gracza $(p)$.
...
Drugi wariant jest identyczny z wariantem 28 parametrów pod względem struktury i zestawu parametrów, ale wprowadza \textbf{normalizację} każdej cechy do przedziału $[-1, 1]$ przed pomnożeniem przez wagę.

Bez normalizacji poszczególne cechy mają bardzo różne zakresy wartości --- przykładowo agregowana wartość stronnika $\nu(m)$ może wynosić nawet kilkaset punktów, podczas gdy zmiana licznika Jade Golem zwykle wynosi co najwyżej 2. To powoduje, że wagi cech o dużych wartościach bezwzględnych dominują w procesie optymalizacji i utrudniają poprawne oszacowanie względnej ważności cech. Normalizacja wyrównuje skale tak, że każda waga $w_k$ wyraża bezpośrednio \emph{ważność} cechy, niezależnie od jej naturalnej skali liczbowej.

Normalizacja obejmuje 28 cech obliczanych ze zmian stanu gry (różnic między stanami przed i po wykonaniu akcji), natomiast funkcja wartości stronnika $\nu(m, \mathbf{w})$~\eqref{eq:minion_value} pozostaje niezmieniona --- jej składniki (zdrowie, atak, zdolności) nie są normalizowane wewnętrznie, ponieważ $\nu(m, \mathbf{w})$ pełni rolę wagi agregującej, a nie bezpośredniej cechy stanu.

Pułap normalizacyjny $C_k$ dla każdej cechy wyznaczono empirycznie. Przeprowadzono turniej między dwoma agentami bazowymi, rozgrywając $\approx1,08$ miliona gier, w których zbierano wartości bezwzględne każdej cechy. Jako $C_k$ przyjęto obserwowane maksimum bezwzględne. Dzięki temu w typowych sytuacjach rozgrywki wartość znormalizowana mieści się w $[-1, 1]$, a wartości ekstremalnie duże (rzadkie) są obcinane do $\pm 1$, nie zaburzając skali.
...
\begin{equation}
    ...
    \label{eq:normalize}
\end{equation}
gdzie $\text{sgn}(\cdot)$ to funkcja signum (znak argumentu: $-1$, $0$ lub $1$), $C_k > 0$ to empiryczny pułap normalizacyjny, a $\tilde{x}_k \in [-1, 1]$. Wartości $C_k$ dla wszystkich 28 cech zestawiono w tabeli~\ref{tab:weights28norm}.
...
12 & Zdolność: agonia                         &   1{,}0   \\
13 & Zdolność: boska tarcza                   &   1{,}0   \\
14 & Zdolność: inspiracja                     &   1{,}0   \\
15 & Zdolność: kradzież życia                 &   1{,}0   \\
16 & Zdolność: ukrycie                        &   1{,}0   \\
17 & Zdolność: prowokacja                     &   1{,}0   \\
18 & Zdolność: furia wichru                   &   1{,}0   \\
19 & Zdolność: trucizna                       &   1{,}0   \\
...
Rozgrywka w Hearthstone przebiega przez trzy strategicznie różne etapy. We wczesnej fazie (tury 1--3) obaj gracze dysponują ograniczoną maną i zagrywają tanie karty, walcząc o kontrolę planszy. Faza środkowa (tury 4--6) charakteryzuje się eskalacją siły stronników i aktywacją kluczowych synergii. Synergia to połączenie kart i mechanik w taki sposób, że wzajemnie się wzmacniają, np. poprzez zmniejszenie kosztu zaklęcia lub zwiększenie ataku lub zdrowia stronnika dzięki innym, już zagranym kartom. W późnej fazie (od tury 7) gracze grają najdroższe, decydujące karty. Agent bazowy stosuje ten sam wektor wag przez całą grę, nie uwzględniając tych strategicznie różnych kontekstów decyzyjnych.

Wariant ten wprowadza \textbf{niezależny wektor wag dla każdej fazy}, definiowanej na podstawie numeru tury:
\begin{equation}
    \text{faza}(t) =
    \begin{cases}
        1 & \text{jeśli } t \leq 3 \\
    ...
    \label{eq:phase}
\end{equation}
Łączna liczba parametrów wynosi $D = 63 = 21 \cdot 3$. Wektor wag $\mathbf{w} \in [0,1]^{63}$ jest konkatenacją trzech niezależnych wektorów:
\begin{equation}
...
\begin{align}
    ...
    \label{eq:smooth3}
\end{align}
gdzie $\delta_{\min} = -0{,}1$, $\delta_{\max} = 0{,}1$, a $\text{clip}(x, 0, 1) = \max(0, \min(1, x))$. Wagi każdej fazy mogą różnić się od wag fazy poprzedniej o co najwyżej $|\Delta w^{(i)}_k| \leq 0{,}1$, co wymusza ciągłość strategii i zawęża przestrzeń przeszukiwań.

\subsection{Wariant 21 parametrów z przeszukiwaniem głębokościowym}
\label{sec:agent21depth}

Agent bazowy stosuje strategię zachłanną z horyzontem 1 --- ocenia akcje wyłącznie na podstawie natychmiastowej zmiany stanu gry, ignorując przyszłe konsekwencje w obrębie bieżącej tury. Tymczasem w Hearthstone wiele silnych kombinacji (ang. \textit{combo}) wymaga wykonania sekwencji akcji, gdzie pierwsza akcja sama w sobie ma niską wartość, ale otwiera możliwość wykonania kolejnej o dużej wartości. Strategia zachłanna nie uwzględnia zatem takich kombinacji --- przykładowo, zagranie taniej karty wzmacniającej (ang. \textit{buff}) przed atakiem stronnika przynosi łącznie wyższy zysk niż sama ocena zagrania wzmocnienia, która bez kontekstu kolejnej akcji wydaje się przeciętna.

Piąty wariant zachowuje bazowy zestaw 21 parametrów, lecz zmienia \textbf{strategię decyzyjną} z zachłannej \eqref{eq:greedy} na przeszukiwanie głębokościowe (ang. \textit{lookahead}) o stałej głębokości $d = 2$ w obrębie tej samej tury. Wartość akcji $a$ wyznaczana jest rekurencyjnie:
\begin{equation}
    F(s, a, \mathbf{w}, d) =
    \begin{cases}
...
Głębokość $d = 2$ oznacza, że agent ocenia bezpośredni efekt akcji oraz najlepszą możliwą kontynuację w tej samej turze. Złożoność obliczeniowa wynosi $O(|\mathcal{A}(s)|^2)$ na każdą turę, co przy typowej liczbie dostępnych akcji $|\mathcal{A}(s)| \approx 10$--$20$ daje od 100 do 400 wywołań symulatora --- akceptowalny koszt w porównaniu z wariantem głębokości~3, gdzie liczba wywołań wzrasta z około $10^3$ do $8 000$.

Wariant ten nie wprowadza nowych parametrów --- wagi $\mathbf{w}$ pozostają identyczne jak w agencie bazowym. Zmiana dotyczy wyłącznie sposobu ich wykorzystania --- zamiast oceniać każdą akcję w izolacji, agent przeszukuje sekwencje dwóch kolejnych akcji i wybiera taką pierwszą akcję, która prowadzi do najwyższej sumy ocen. Dzięki temu możliwe jest wykrycie kombinacji niewidocznych dla strategii zachłannej, bez zwiększania liczby optymalizowanych parametrów.
