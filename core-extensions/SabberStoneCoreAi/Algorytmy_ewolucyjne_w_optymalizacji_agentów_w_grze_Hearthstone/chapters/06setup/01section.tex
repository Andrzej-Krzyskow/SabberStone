...
Niniejsza sekcja zawiera formalny opis agenta parametrycznego, będącego podstawą wszystkich modyfikacji. Notację ujednolicono z opisem rozszerzeń przedstawionych w dalszej części pracy. Notacja ta może ona różnić się od notacji stosowanej przez García-Sánchez i in.~\cite{sanchez2020}.
...
Zbiór $\mathcal{K}$ zawiera dziewięć zdolności specjalnych stronnika (nazwy polskie zgodne z tabelą~\ref{tab:weights21}):
\[
    \mathcal{K} = \{
        \text{Szarża},\;
        \text{Deathrattle},\;
...
Bazowy agent posiada $D = 21$ parametrów. Pełny wektor wag $\mathbf{w} \in [0,1]^{21}$ zestawiono w tabeli~\ref{tab:weights21}. Wektor $\mathbf{w}$ stanowi \textbf{genom} osobnika w algorytmie ewolucyjnym. Przestrzeń przeszukiwań to $[0,1]^D$, która to wyznaczana jest przez każdego agenta.
...
18 & $w_{\text{Windfury}}$      & Zdolność: Furia Wiatru \\
