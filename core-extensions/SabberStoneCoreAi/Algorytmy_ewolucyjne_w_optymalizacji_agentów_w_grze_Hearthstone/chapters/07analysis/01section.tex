...
\textbf{Faza~III} to konfrontacja zwycięzcy turnieju głównego z agentem zewnętrznym z konkursu \textit{Hearthstone AI Competition}~\cite{hearthstone_competition}. Pozwala to ocenić, jak proponowane podejście wypada na tle innych rozwiązań.
...
\begin{equation}
    \text{WR}(A) = \frac{\text{wygrane}(A)}{\text{liczba partii rozegranych przez } A} \cdot 100\%.
    \label{eq:winrate}
\end{equation}

Wskaźnik zwycięstw każdego agenta liczony jest względem wszystkich jego partii w turnieju, a nie tylko względem jednego przeciwnika. Dzięki temu wynik jest mniej wrażliwy na asymetrię konkretnych zestawień talii.
