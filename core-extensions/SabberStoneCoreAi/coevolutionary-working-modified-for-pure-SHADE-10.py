...
NUM_GAMES = 20 #8
POP_SIZE = 10 #10
MAX_EVALUATIONS = 820 #1000
NUM_THREADS = 128  # 8
NUM_WEIGHTS = 21
TEMP_FILE_NAME = "results.tmp"
TEST_DUMMY = False

OPPONENTS_CSV_ORIGINAL = "opponents-10.csv"
...
def shade_observer(generation, population, population_fitness, mean_f=0.5, mean_cr=0.5):
	...
	if generation == 0:
		...
		_log_files['stats'] = open('shade-pure-10-statistics-file-{}.csv'.format(ts), 'w')
		_log_files['indiv'] = open('shade-pure-10-individuals-file-{}.csv'.format(ts), 'w')
		_log_files['matrix'] = open('shade-pure-10-matrix-file-{}.csv'.format(ts), 'w')
		_log_files['coeff'] = open('shade-pure-10-coefficients-file-{}.csv'.format(ts), 'w')
